    ...
    try {
      ...
      if (!failureType && FRONTEND_PATHS.has(endpoint)) {
        ...
        if (assetPath) {
          ...
          } catch { failureType = "frontend_asset_unreachable"; }
        }
      }
      results.push({ timestamp, endpoint, status: response.status, response_headers: Object.fromEntries(response.headers), deployment_id: deploymentId(response.headers), vercel_id: response.headers.get("x-vercel-id") || "", body_excerpt: excerpt(body), failure_type: failureType, pass: !failureType });
    } catch (error) {
      results.push({ timestamp, endpoint, status: 0, response_headers: {}, deployment_id: "", vercel_id: "", body_excerpt: excerpt(error.message), failure_type: error.name === "TimeoutError" ? "timeout" : "network_error", pass: false });
    }
  }
  return results;
}
