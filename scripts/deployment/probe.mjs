...
export async function probe(baseUrl, { endpoints = PRODUCTION_ENDPOINTS, timeoutMs = 15000, fetchImpl = fetch, attempts = DEFAULT_ATTEMPTS, retryDelayMs = DEFAULT_RETRY_DELAY_MS, protectionBypassSecret = process.env.VERCEL_PROTECTION_BYPASS_SECRET || "" } = {}) {
  const results = [];
  const headers = { "user-agent": "menuply-external-watchdog/1.0" };
  if (protectionBypassSecret) headers[PROTECTION_BYPASS_HEADER] = protectionBypassSecret;
  for (const endpoint of endpoints) {
    ...
    while (attempt < attempts) {
      ...
      try {
        const response = await fetchImpl(new URL(endpoint, baseUrl), { redirect: "follow", signal: AbortSignal.timeout(timeoutMs), headers });
        ...
        if (!failureType && FRONTEND_PATHS.has(endpoint)) {
          ...
          if (assetPath) {
            try {
              const assetResponse = await fetchImpl(new URL(assetPath, response.url || new URL(endpoint, baseUrl)), { redirect: "follow", signal: AbortSignal.timeout(timeoutMs), headers });
