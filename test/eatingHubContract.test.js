...
test("My Menuply and peer hub use five-section presentation hub", () => {
  ...
  assert.match(section, /placement="header"/);
  ...
  assert.match(
    mine,
    /previewAsConnect|get\("view"\) === "connect"|URLSearchParams\(window\.location\.search\)\.get\("view"\)/
  );
  ...
  assert.match(bits, /testIdPrefix="want-to-eat"/);
