...
test("See Who's Eating reel: guest watch, CK dish, existing camera, Connect notify", () => {
  ...
  assert.match(fullscreen, /tearDownFeedVideoElement/);
  assert.match(fullscreen, /clearStuckMediaChrome/);
  assert.match(fullscreen, /useLayoutEffect/);
  ...
  assert.match(read("src/lib/feedVideoPresentation.js"), /tearDownFeedVideoElement/);
  ...
  assert.match(fullscreen, /feed-desktop-rail-w|desktopFeedShell/);
  // Feed home must not portal to document.body (leftover cover kills Profile bottom nav).
