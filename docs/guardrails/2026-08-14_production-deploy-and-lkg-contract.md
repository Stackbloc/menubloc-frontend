...
## CURRENT LAST KNOWN GOOD (live production — 2026-09-21)
...
| Git commit | `3ad4116c` — USC On campus dining-hall status copy — USC Coffee & Bakery name match + exclude Yogurtland — standard marketplace commission 6% — video manager SEO Cluster dropdown — cluster Food search price sort parity — Menu Manager multi-file continue-on-error — replace-media clears thumb + recapture nudge — video manager replace-file + sitemap lastmod harden — video search metadata admin v1 PR2/PR3 sitemap + profile VideoObject — profile diary text-only + Videos B9/B11 sizing — restaurant profile Videos tile grid + sheet player — SEARCH-001 contextual Connect/Events sections; global price+who-has — diner venue-claim copy + Month in Food freshness — Month in Food editorial redesign — Month in Food Moments-only + overflow + spend UX; empty-menu places — Whats cookin @home: prefix + customer-facing @home copy — Current vibe Open for suggestions default + Catch Me + VM food-type tree — Current vibe Open for suggestions default + Catch Me + VM food-type tree — Current vibe Open for suggestions default + Catch Me + VM food-type tree — FE tip sync same bundle (Invite CTA is BE); vibe prominence — Current vibe under Favorite foods + events count filter — Current Vibe avatar badge + picker on profile — prior-day What I'm Eating journal Add — menu-item video review + Connect search strip — Video Manager cuisine picker + Asian food types — Video Manager food-type picker + canonical-menu place pickers — Search video hide empty shell; VM thumbnail + Part 2/3 — unified search-result card Video→Connect→Deal stack — search result video strip from payload videos — edit extras one meal + dish name persists — mobile dialogs + eating full edit — Catch Me editor card + cluster billboards — diner Catch Me + Waiter LDL — franchise brand location for banner and menu — What's cookin' long-press Edit — waiter structural personalization — Happy Hour Who's Eating + long-press Edit + unclaimed profile copy — Multiplier camera Record video viewport — Join Me brand green + cravings header placement — JoinMeButton Connect; Edit Join Me status per occasion — quiet Join Me status line no green pills — per-event Turn on Join Me edit — per-instance Join Me plans+events — profile SectionHeader eight sections — live tip includes Edit View max-update-depth cc363ff1 — live tip after social-events; includes Edit View max-update-depth cc363ff1 — Edit View max-update-depth — Edit View max-update-depth: stable social-preset empties so nav remounts content — Feed first video: stop clearStuck killing live reel; header z1300 — Feed Edit-view + Deals/modal nav harden (tearDown + overflow restore races + CLEAR_STUCK) — mobile Feed playing → Profile nav (body portal + reel teardown) — Feed-as-home single shell: / redirects to /feed — Feed nav + Multiplier dismiss (lock after minify content-probe miss) — Feed home reel no body portal — Profile bottom nav — Happy Hour compose + chronological What I'm Eating order — Happy Hour shared Multiplier/Edit compose — Edit/Connect multi-toggle state fix — Edit/Connect label + Waiter Hello comma — restore Edit/Connect header; finish incomplete content-probe CPD — Edit/Connect + MMIF chrome above stuck media overlays — Highlights Save + endless Feed wrap — unified multi-item meals — Video Manager preview mute syncs to Feed play_muted — finish section Edit View presets CPD — VM Stop Feed tip lock — quiet Edit View audience presets + All Connects — Video Manager modal ink + Stop showing in Feed — Edit View presets + unified meal + Month in Food — Video Manager modal ink + Stop showing in Feed — Edit View presets + unified meal + Month in Food — CSP connect-src allow Supabase signed PUTs — campus Venue profile video billboard /venues/:slug — Video Manager signed direct Supabase upload — My Highlights IG grid photo+video; See all page — Cause 2 signed Supabase direct diner video upload — My Highlights diner-owned photos; Edit View + for Events/Crews/Highlights — Edit/Connect View names + Wanna Eat Join Me + follow card parity — diner video async normalize + upload progress/errors — Who's Eating menu-item not restaurant dish — Who's Eating continuous line + affiliation — detail/search city-state geo for cmi (Dothan fix) — feed video playback: geo then shuffle, not upload chrono — diner video: 5m timeout, no length-blame, soft too-long; Connect Join Me gone — Video Manager mute/run/inactive + drop Today kicker on What I'm Eating — timezone-safe journal calendar days What I'm Eating — eatingFoodName src/lib all compose/media paths — eatingFoodName in src/lib for all Wanna Eat/media compose paths — Wanna Eat eatingFoodName import fix — Month in Food profile Wanna Eat / Join Me / My Events sync — desktop glasses view toggle next to settings — Connect View Join Me/Take Me Out + glasses view toggle — MMT inbox + confirmed booking UX; missing-request copy — feed: no Remove from Public Feed on own clips; delete via profile long-press — feed: no Remove from Public Feed on own clips; delete via profile long-press — restore Join Me/Take Me Out under Wanna Eat; TikTok video duration policy — Make Me This opt-in offer schedule — Join Me Take Me Out + Top Highlights — honor billboard Image Fit contain for full building — What I'm Eating Meal n line format — splash default restaurant name; custom splash text — profile Edit last; empty about hide; smaller highlights — What's Cooking @home; @home photo +; Connect content-only; Wanna Eat names; drop Connects strip — venue photo top-anchor + suppress Entrance billboard splash — Connect-view prose + Wanna Eat Actions + crew affiliation/organizer — profile modernization: compact Add + connect preview — activity logo/billboard + clickable dish/place — category status-line compose + prose activity — fix /feed/profile blank: socialBtn.primary — quick food status + derived emoji scan; 4 meal periods — quick food status + derived emoji scan; 4 meal periods — CK What I'm Eating emoji compose; video same scan rows — activity-first scan rows + sticky text composer — drop Today from profile hours heading — Waiter info-only activity + 3 meal options — profile activity-selection layer — New on Menuply cuisine + expense — Wanna Go! copy rename — unified business signup + More drawer Clusters — Restaurant signup free invite + Terms; no commission — Waiter contrast + remove Optional Paid Upgrades — Waiter briefing rebuild + Cravings Invite sheet — feed: profile auth return; Wanna Go plate; Share My QR; Added — feed: remove Save menu; Share QR nav; Menu Browser on rail — feed rail mobile-only; nav Waiter; Connect first — feed video action rail + Waiter — invite funnel A-D: pricing claim FT dashboard — restaurant invite signup + Terms v2026_09_07 — profile Videos: initial 3, 9:16 cover grid, shuffle on load — Online Ordering Available chip invert — Multiplier/Post rail + sheet label — lock live tip after STALE_LOCK; includes owner upload clearer-photo UX — Multiplier rename + persistent top menu highlight — menu highlight expiry scroll to restaurant top — restaurant menu contextual video PiP — menu-upload OCR NDJSON heartbeats — Feed Menu Browser nav → PiP like yellow icon — Menu Browser PiP polish + Feed tab Yellow Browse — Menu Browser trail swipe + PiP video advance — Menu Browser PiP frozen browse + mini-player — Menu Browser PiP frozen browse + mini-player — Feed Yellow Browser icon → restaurant menu — Bookmarked Menus CPD; live tip after concurrent menu-scan 100MB ship — menu-scan upload limit 100 MB — Bookmarked Menus + Yellow Browse search/explore CPD — Bookmarked Menus + Yellow Browse search/explore — Wanna Go restaurants under What I Wanna Eat — Join Me RSVP path + hub copy + crew member chips — discoverable diner profile Connect/Block/Report + plan SEO exclusion — Platform video caption once (no duplicate under poster) — Find Diners member discoverability — no member # — Northern Cafe: hide Add Menu when menu has items — Connect peer blank + DOB favorites + @home camera inline — profile hub Who's Eating camera MMT picker — video watch SEO + JSON-LD + destination venue parity — Menu Manager Photos library multi-select (split PDF) — diner food intent Phases 1-8 Meal Intel Intent-Based Offers — restore Menu Manager upload picker to pre-Aug 5 (single multiple input) — View Menu displays menu for guests; upload signed-in only — placeholder menus flip to View Menu when items are added — profile Videos player ~half size — owner Menu Manager upload Failed-to-fetch hardening (timeout + JPEG/PNG/WebP) — owner Menu Manager phone photo multi-select (split PDF picker + contract) — View Menu for unclaimed restaurants with published items (Bacari West Adams) — @home camera hover: take photo or media under X — Flash Video on My Menuply About Me + @home camera icon — What I'm Cooking Feed X last Post to Feed — @home meal photo grid on My Menuply profile — @home dish grid + Flash Video on /diners/:userId — Share & Invite on feed and deals videos — deal video audio + desktop caption dock — Owner video upload Failed-to-fetch hardening — Knowledge Bot URL parse + form hydrate (re-certify) — Knowledge Bot URL menu parse + form hydrate — growth details LA TZ — Knowledge Bot agent-install OCR/MLE UI — visitor analytics Unattributed reconcile — feed restaurant name tap opens profile page — Platform video Feed poster labels + owner Video Manager creator column — owner deal video upload panel + platform video creator label — Feed desktop arrow-key navigation copy — My Menu Stack liked-restaurant tier + diner instructions — Owner Knowledge Bot in Control Center — Feed default audio on; Video Manager modal metadata editor — Desktop Feed click-for-sound unmute — Feed video desktop framing, tap-for-sound, mic capture, place caption — Owner video catalog CK pickers + date filters (Easy Street BE seed already live) — Owner video catalog CK name search + created date range — Diner search + Make Me This profile display — owner video catalog CK pickers — diner hobbies collapsed personal details — collapse personal context editor behind toggle — inline personal context editor on profile — profile settings links discoverability — diner personal context profile header lines — owner Video Catalog console + asset numbers for all Feed videos — LGD Let's Get Drinks quick invite with optional menu attachment — Make Me This private I Wanna Eat requests — guest Post to Feed video Terms gate + signup invite — Billboard unauthorized asset removal — tip lock sync — Remove unauthorized scraped billboard assets — Billboard durable assets Tom's Emmy + systemic repair — Merge Home into Dishes on profile + Month in Food — Fixins billboard asset + tip sync — Indio Festival Grounds directory card name fix — Indio H1 SEO overlay pre-migration 0303 — Indio Festival Grounds tradename de-risk; BE 69b70d3b — Indio Festival Grounds tradename de-risk Option C — Indio Festival Grounds tradename de-risk Option C — Stats bar 5-col one line; Month in Food Home; Include DIY recipes search — billboard Tom's crop splash dedupe optional promo headline — operator billboard preview URL resolution — Menu Manager OCR source rail restore — My Menuply Home tab; remove profile homemade share CTA — Homemade Dishes: compose, detail, search Include Homemade, Show Me How — dining-crew reader vs member detail page split — My Menuply hybrid section polish + join-only profile cards — crew invite Connections picker + Find Diners UX — billboard FK media URLs + Klaudette mobile crop — sample menu stack + QR-first Share My Menuply — restaurant profile refinement + My Menuply /feed/profile hub — restaurant dining intent CPD finish — restaurant People who want to go dining intent — feed shareable videos+profiles; account invite on receive; quick invites — billboard 10 MB upload limit copy — owner Profile Manager billboard Windows photo upload — Feed X Upload media category picker E2E — Feed menu upload camera rail tooltip mobile icon — desktop Feed Log in Menuply green — Feed footer trim, My Menu Stack, X coach, desktop Home order — Feed Reviews + Food Review X + Share My Menuply rail + Shop footer polish — Feed desktop blank hotfix + full HomeNext Shop tab restore — Feed Shop HomeNext + shell basket alignment — Feed Shop tab, deals desktop fix, More menu, green login — Feed discovery rail, More menu, signup screen name — Feed default home at /; existing FeedPrimaryNav unchanged — Connect presumed name: First L. coalesce + signup names — Connect display names: First L. instead of Member # — Feed Search hero vertical spacing — Feed Share My Menuply → Diner QR page — Feed Search no menu windows + in-shell results — Feed Profile: profile-only, no hero/TV — Feed TikTok nav + slim X sheet — Feed Menus replaces Eating + mobile-first empty coach — Feed first-visit empty coach + counsel Terms/Privacy tip lock — Counsel-approved Terms+Privacy UGC Aug 27 2026 — Owner Feed Invite QR image credentialed fetch fix — Feed deal video swipe: meal-time filters + Lunch Deal caption + restaurant feed compose — Feed deal video swipe reel; text search on /deals — Feed X categorized menu: My Menuply, Share, Account — Owner Feed Invite QR editable poster copy — Feed Deals Live meal periods + media — feed eating tab real compose actions — eating restaurant + menu item fields for ate/want — feed deals chrome + me clusters/menu upload — feed video-only hybrid ConsumerCameraSheet recorder — feed center X video create + plan video attach — video-first parallel `/feed` shell (FEED\|EATING\|EVENTS\|ME); HomeNext remains `/` |
| Vercel deployment | `menubloc-frontend-4ml2lokz1-menuply.vercel.app` |
| Live bundle | `index-BDuM_ubd.js` |
...
| Git on origin | `f74cd0ec` — OCR screenshot chrome strip + newline rebuild (Il Giardino A+B) |
| Live health SHA | `f74cd0ec847a0d15157c160a7c04d4b4424e19c1` |
| Health URL | `https://menubloc-backend-production.up.railway.app/health` |
| `commit_hash` | `f74cd0ec…` |
| DB | migrations through `0335` (`video_asset_metadata.cuisine`) |
| Smoke | OCR A+B CPD 2026-09-21 (smoke 30, health `f74cd0ec`) |

### Restore current tip (if tip-gate fails mid-change)

```bash
cd /Users/andrebarber/Desktop/menubloc/menubloc-frontend-main
npx vercel alias set menubloc-frontend-4ml2lokz1-menuply.vercel.app menuply.com
npx vercel alias set menubloc-frontend-4ml2lokz1-menuply.vercel.app www.menuply.com
npx vercel alias set menubloc-frontend-4ml2lokz1-menuply.vercel.app crm.menuply.com
npx vercel alias set menubloc-frontend-4ml2lokz1-menuply.vercel.app venues.menuply.com
...
| Field | Value |
|-------|-------|
| Deployment | `menubloc-frontend-eb63gjdit-menuply.vercel.app` |
| Live bundle | `index-DsXAAWzZ.js` |
