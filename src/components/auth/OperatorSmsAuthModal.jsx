...
export default function OperatorSmsAuthModal({ open, onClose, onSuccess, sendSmsCode, verifySmsCode }) {
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => setResendTimer((v) => Math.max(0, v - 1)), 1000);
  ...
  return (
    <div
      style={{ position: "fixed", inset: 0, zIndex: 1400, background: "rgba(15,23,42,0.52)",
               display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}
      onClick={() => onClose?.()}
      role="presentation"
    >
      <div
        style={{ width: "100%", maxWidth: 420, borderRadius: 24, background: "#fff", padding: 20,
                 border: "1px solid rgba(15,23,42,0.08)", boxShadow: "0 28px 72px rgba(15,23,42,0.28)" }}
      ...
      >
        ...
        {step === "phone" ? (
          <form onSubmit={handleSendCode} style={{ marginTop: 18, display: "grid", gap: 12 }}>
            <input
              ...
              style={{ width: "100%", boxSizing: "border-box", borderRadius: 14,
                       border: "1px solid #d0d5dd", padding: "12px 14px", fontSize: 16, background: "#fff" }}
            />
            <button
              type="submit"
              disabled={loading}
              style={{ border: "none", borderRadius: 16, background: "#11211a", color: "#fff",
                       padding: "14px 16px", fontSize: 15, fontWeight: 900, cursor: loading ? "wait" : "pointer" }}
            >
          ...
          <form onSubmit={handleVerifyCode} style={{ marginTop: 18, display: "grid", gap: 12 }}>
            <input
              ...
              inputMode="numeric"
              style={{ width: "100%", boxSizing: "border-box", borderRadius: 14,
                       border: "1px solid #d0d5dd", padding: "12px 14px", fontSize: 20,
                       letterSpacing: 6, textAlign: "center", background: "#fff" }}
            />
            <button
              type="submit"
              disabled={loading}
            ...
            <button
              ...
              style={{ border: "1px solid rgba(17,33,26,0.12)", borderRadius: 16, background: "#fff",
                       color: "#11211a", padding: "12px 14px", fontSize: 14, fontWeight: 800,
        ...
        <button
          type="button"
          onClick={() => onClose?.()}
          style={{ marginTop: 16, border: "none", background: "transparent", color: "#667085",
