...
/** Location line for restaurant place hits (city/address only — no internal jargon). */
export function restaurantPlaceMeta(row) {
  return [row?.address_line1, row?.city, row?.state].filter(Boolean).join(", ");
}
...
export async function uploadPublicFoodActivityPhoto(file) {
  const form = new FormData();
  form.append("photo", file);
  form.append("guest_key", getOrCreateGuestReporterKey());
  return apiPostForm("/public/food-activity/photo", form);
}
