 ...
 * Shows similar items and compare when the basket is empty.
 * Similar items section is hidden while the basket has items so
 * the user stays focused on the current restaurant.
 *
 * Used when a user taps a menu item from a restaurant menu page.
 * Search/discovery clicks use MenuItemDetailPage.
 * ============================================================
 */
    ...
    <PageShell isMobile={isMobile}>
      ...
      {!isMobile && showItemPhoto ? (
        <Surface style={{ marginTop: 16, padding: 0, overflow: "hidden" }}>
          <div style={{ minHeight: 320, borderRadius: 22, overflow: "hidden", border: "1px solid #1F2937", background: "#efe9dc" }}>
            <img src={item.itemPhotoUrl} alt={`${item.name} photo`} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
          </div>
        </Surface>
      ) : null}
