...
export default function CrmSeedExplorer() {
  ...
  const [filters, setFilters] = useState({ market_id: "", search: "", page: 1, page_size: PAGE_SIZE });
  ...
  useEffect(() => {
    if (!filters.market_id) return;
    let active = true;
    setLoading(true);
    setError("");
    getCrmSeedRestaurants(filters)
      .then((json) => { if (active) setData(json); })
      .catch((err) => { if (active) setError(err.message || "Unable to load seeded restaurants"); })
  ...
  return (
    <CrmPage title="Seed Explorer">
      ...
      <CrmCard
        ...
        subtitle="Browse core restaurant data by market. Restaurants enter the CRM pipeline only when Add Lead is selected."
      >
        <div style={filterStyle}>
          <label style={labelStyle}>
            ...
            <select
              ...
              style={inputStyle}
            >
              {!markets.length ? <option value="">No active markets</option> : null}
              {markets.map((market) => (
        ...
        <div style={{ marginTop: 18 }}>
          <DataTable
            ...
            emptyLabel={loading ? "Loading seeded restaurants…" : "No seeded restaurants match these filters."}
            columns={[
...
function Pagination({ pagination, onPageChange }) {
  const totalPages = Math.max(1, Math.ceil((pagination?.total || 0) / (pagination?.page_size || PAGE_SIZE)));
  return (
