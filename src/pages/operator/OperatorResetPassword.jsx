...
export default function OperatorResetPassword() {
  ...
  useEffect(() => {
    ...
    validateOperatorResetToken(token)
      .then((result) => {
        if (!cancelled) { setEmail(result.email || ""); setTokenState("valid"); }
      })
      .catch((err) => {
        if (!cancelled) { setError(err.message || "Reset link is invalid or expired"); setTokenState("invalid"); }
      });
  ...
  if (tokenState === "invalid") {
    return (
      <AuthPageFrame
        ...
        footer={<p style={styles.footer}><Link to="/operator/recover" style={styles.link}>Request a new link</Link></p>}
      >
        <div />
      </AuthPageFrame>
    );
  ...
  return (
    <AuthPageFrame
      ...
      subtitle={email ? `Resetting password for ${email}` : "Choose a new password for your operator account."}
      footer={<p style={styles.footer}><Link to="/operator/login" style={styles.link}>Back to sign in</Link></p>}
    >
      <form onSubmit={handleSubmit} noValidate style={styles.form}>
        <PasswordField
          id="op-new-password" label="New password" autoComplete="new-password"
          value={password} onChange={(e) => { setPassword(e.target.value); setError(""); }}
          placeholder="At least 8 characters"
        />
        <PasswordChecklist password={password} />
        <PasswordField
          id="op-confirm-password" label="Confirm password" autoComplete="new-password"
