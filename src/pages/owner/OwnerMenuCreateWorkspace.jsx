...
function isOwnerUploadHeic(file) {
...
function ownerUploadHeicMessage(file) {
  return `${file.name}: HEIC/HEIF is not supported. In Photos, choose JPEG/PNG, or export as JPEG before uploading.`;
}
...
function applyOwnerUploadPick(event, { setFiles, setUploadMsg }) {
  ...
  const nonHeic = picked.filter((file) => !isOwnerUploadHeic(file));
  const tooBig = nonHeic.filter((file) => Number(file.size) > MAX_MENU_UPLOAD_BYTES);
  const ok = nonHeic.filter((file) => Number(file.size) <= MAX_MENU_UPLOAD_BYTES);
  if (ok.length) {
    setFiles((prev) => mergeOwnerUploadFiles(prev, ok));
  }
  const errors = [
    ...heic.map(ownerUploadHeicMessage),
    ...tooBig.map(ownerUploadTooLargeMessage),
  ];
  if (errors.length) {
    setUploadMsg({
      ok: false,
